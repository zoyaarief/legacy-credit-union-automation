...
The system has four control planes around the deterministic executor: discovery and compilation, artifact review and catalog invocation, live execution and handoff, and durable security operations. Discovery scans the current DOM into a sanitized inventory of generic controls, semantic labels, local context, and locator candidates. The provider chooses observation-local references and ordered locator candidates, and compilation accepts only the resulting verified live trace. There is no discovery target-id catalog or fixed workflow locator map. Generated or restored artifacts enter `draft` and cannot replay until the signed-in reviewer approves their SHA-256 fingerprint. The bundled reviewed baseline and its reviewed tenant variants remain executable. D1 stores sanitized approvals, artifacts, rate windows, operational events, outcomes, surface snapshots, and encrypted evidence behind the authenticated Sites user id.

The server provider uses the OpenAI Responses API with strict structured output and `store: false`. If no API key is configured, the server explicitly authorizes a labeled policy-equivalent simulator fallback. Network errors, malformed fallback responses, and other provider failures remain visible instead of silently becoming simulated success. The target and member data are local and synthetic.
...
Compilation accepts only the complete five-action savings lookup trace. Each actionable trace entry retains exactly two locators selected from the candidates derived from its observed element; arbitrary or unobserved selectors are rejected. This prevents a plausible but incomplete model response from becoming executable automation. Approval binds to canonical sanitized artifact content, so any content change produces a different fingerprint and returns to review.

## Determinism & error handling

Discovery and replay validate their contracts before acting. The current discovery policy positively classifies only exact read-only member savings-balance and account-status request forms; every non-matching or compound intent returns `unsupported_goal` before the target is prepared. This avoids depending on an open-ended mutation-word blacklist. Both engines enforce the same-origin `/legacy` allowlist, action and step limits, timeouts, output requirements, known business outcomes, and checkpoint verification. Provider decisions are rejected when they reference hidden, disabled, unknown, or role-incompatible controls, undeclared inputs or outputs, or locators not found in the selected control's current observation. Runtime locator resolution rejects hidden and ambiguous matches, requires enabled controls for input and click actions, and records the successful locator used for extraction. Tests also run discovery against a reordered surface with opaque references, distractor controls, and different field, button, result, and cell locators; the compiled artifact contains those observed locators rather than main-tenant constants.
...
`DiscoveryAdapter` and `SurfaceAdapter` isolate the implemented web mechanics from planning and execution. Target readiness is inferred from a stable set of visible, interactive controls; the mock application exposes no private automation-ready signal, and its result, error, and intervention containers have no stable automation IDs. The current artifact schema and locators are explicitly web-only; desktop or accessibility-tree support would require a new surface-specific schema plus adapter rather than reusing this artifact unchanged. The catalog keys the capability to `northstar-core-member-services@8` and resolves only static reviewed web tenant profiles. The east-branch profile changes the entry point and two ordered locators while preserving inputs, outputs, policy, business outcomes, and checkpoint semantics.
...
The implemented ownership states are `automation`, `human_requested`, `human`, `resuming`, and `completed`. During discovery or replay, a restricted-account interstitial raises an intervention containing the exact stopped step, reason, and a redacted structured surface snapshot. The active engine pauses before the blocked action and preserves its cursor and iframe.

After an operator accepts, the existing iframe becomes the manual control surface. Click and input events are captured by control name only; values are never recorded. Discovery and replay resume explicitly, skip session preparation, revalidate the current URL, continue from the stopped wait step, and verify their original outputs and checkpoint. The complete evidence sequence is stored under the same run id.

## Safety

The capability is read-only. Runtime policy enforces target path, action allowlist, step budget, total timeout, input shape, declared extraction fields, and final checkpoint. Discovery inputs stay in the browser adapter and are replaced with placeholders before provider calls and evidence recording. Agent invocation inputs cross only the authenticated issue/verify boundary, are signature-bound, and never enter telemetry or run evidence. A provider may order two selectors only from the candidate set derived from the control it selected in the current observation; it cannot invent selectors or carry observation references into another surface state.
...
The submitted evidence is generated directly by the discovery and replay engines and includes ordered event arrays, timestamps, decision reasons, successful locators, outputs, and checkpoints. Discovery now emits live-surface candidate version 1.2.0; deterministic replay evidence retains the separately reviewed bundled baseline version 1.1.0, reflecting the required draft-and-approve boundary instead of implying automatic promotion. The private demo still has one granted Site owner, so a real two-person vote cannot be demonstrated until another authenticated Site visitor is granted. No external webhook or platform scheduler destination was provided, so those integrations remain configuration-gated. Desktop automation, fully unattended browser execution, and broader network faults remain deliberate cuts.
